...
$$|x\rangle$$
表示位置的基底。對量子系統量測，也等同於對波函數量測。量子力學的量測本身就是一個巨大的開放問題，我們這裡就先不深入討論，只描述對波函數量測另外一個重要的性質。量子力學中，量測過後的波函數，會因為量子塌陷而變成量測完的狀態。也就是說量測前系統可以處在$x=0$『加上』$x=0.5$ 這種狀態。量測做後如果得到量測結果是$x=0$，則表示量測後的系統是百分之百在$x=0$的狀態。這個過程就是量子塌陷。
